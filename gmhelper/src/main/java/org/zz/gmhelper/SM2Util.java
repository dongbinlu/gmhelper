...
public class SM2Util extends GMBaseUtil {
    ...
    /**
     * 只获取私钥里的d值，32字节
     *在SM2算法中，私钥由两个部分组成：d 值和公钥。d 值是私钥中的一个重要组成部分，它是一个随机选择的数值，用于生成公钥和进行数字签名。
     *
     * 具体来说，私钥由一个整数 d 和一个椭圆曲线上的点组成。d 值是一个随机选择的整数，它是椭圆曲线离散对数问题的关键部分。使用 d 值，可以通过椭圆曲线上的运算生成公钥，并且可以用于对消息进行数字签名和解密。
     *
     * 在 SM2 算法中，d 值是保密的，应当严格保护。泄露了 d 值将导致私钥的暴露，可能会对数字签名的安全性造成严重影响，因此在实际应用中需要特别注意保护私钥的安全性。
     * @param privateKey
     * @return
     */
     ...
     * @param srcData          原文
     * @return 默认输出C1C3C2顺序的密文。C1为65字节第1字节为压缩标识，这里固定为0x04，后面64字节为xy分量各32字节。C3为32字节。C2长度与原文一致。
     * 注意这里的xy可不是公钥里的xy，而是在加密的过程中生成生成一个临时随机数K，然后使用接收者的公钥点（x,y）计算出C1，C1是椭圆曲线上的一个点，
     * 但并不等于接收者的公钥点。
     * @throws InvalidCipherTextException
     */
     ...
     * @return DER编码后的密文
     *
     * 将SM2密文转换为der编码，其实就是asn1编码，通过Asn1View工具查看为一下四部分：
     * INTEGER---->表示X值，长度为33，其实X值长度为32位，可以看到第一个字节为00
     * INTEGER---->表示Y值，长度为33，其实X值长度为32位，可以看到第一个字节为00
     * OCTET STRING---->C3,校验码，长度为32，SM3计算得出
     * OCETE STRING---->C2，密文，加密后的密文数据，长度和原文一致。
     *
     *
     *
     * @throws IOException
