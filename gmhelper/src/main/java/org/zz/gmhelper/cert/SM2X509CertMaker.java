...
public class SM2X509CertMaker {
    ...
    /**
     * User
     * 生成根CA证书和生成SubCA证书的区别
     *
     * 生成根CA（Certificate Authority，证书颁发机构）证书和生成子CA证书之间有一些重要的区别：
     *
     * ### 根CA证书生成：
     *
     * 1. **根证书的自签名：** 根CA证书是整个证书链的顶级，它自身签署自己的证书。这意味着根证书的签名由自身的私钥生成，没有其他CA签署它。根证书通常是信任链的根基。
     *
     * 2. **信任和分发：** 根证书应该被信任，并且需要安全地分发给用户或应用程序，以便将它添加到信任存储中，使得其他证书（包括中间CA证书和终端证书）能够被验证和信任。
     *
     * ### 子CA证书生成：
     *
     * 1. **由根CA签署：** 子CA证书是由根CA签署的，它们不是自签名的。通常情况下，根CA签署子CA的证书请求来创建子CA。
     *
     * 2. **层次结构：** 子CA证书可以用来签署终端实体（例如用户或服务器）的证书，也可以用于签署其他中间CA证书。这样的层次结构使得在更大范围内管理和组织证书成为可能。
     *
     * 3. **授权范围：** 子CA证书可能限制其签署的证书类型和用途。它可能具有特定的策略和限制，例如，可能只能用于特定的服务、特定的域名或特定的用途。
     *
     * 在整个PKI（Public Key Infrastructure，公钥基础设施）中，根CA扮演着信任根源的角色，而子CA通常用于在更局部的范围内签署证书，并且可以根据需要管理和分配不同级别的信任和权限。
     */
